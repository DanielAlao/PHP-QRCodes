...
class Users
{
    ...
    {   
        // Only admin admin is allowed to access this page
        if ($_SESSION['user_type'] !== 'admin')
            $this->failure('Only a "admin" account can access the admin listing page', 'Location: index.php');
    }
    ...
    public function add()
    {
	    ...
	    // Reset db instance
	    $db = getDbInstance();
	    $last_id = $db->insert('user_accounts', $data_to_db);

	    if ($last_id)
		    $this->success('User user added successfully');
    ...
    public function edit()
    {
	    ...
	    $db = getDbInstance();
	    $db->where('id', $admin_user_id);
	    $stat = $db->update('user_accounts', $data_to_db);
